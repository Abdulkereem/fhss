...
def signup():
    ...
    if mode == 'Private':
        ...
        try:
            ...
            new_enrollee = Enrollee(fhss=fhss, mode_of_reg=mode, sector=sector, first_name=fname,
...
@login.route('/hcp/join', methods=['GET','POST'])
def create():
    if request.method == 'POST':
    ...
    return render_template('./hcp/signup.html')
...
def hmosignup():
    try:
        ...
        return redirect(url_for('hmos.hsignup'))
    except IntegrityError:
        flash('sorry user already exist')
        return redirect(url_for('hmos.hsignup'))
